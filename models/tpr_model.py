...
from utils import transforms as extend_transforms
...
class Model(object):
    ...
    def _create_net(self):
        ...
        self.net = nets.__dict__[model_name](color_channels=color_channels, feature_size=27, num_classes=num_classes)
    ...
    def _create_dataset(self):
        ...
        train_dataset = ThyroidDataset(root_dir=data_params['train_dir'],
                                pkl_file=data_params["train_file"],
                                mode = "train",
        ...
        eval_dataset = ThyroidDataset(root_dir=data_params['train_dir'],
                                pkl_file=data_params["train_file"],
                                mode = "test",
        ...
        val_dataset = ThyroidDataset(root_dir=data_params['val_dir'],
                                pkl_file=data_params["val_file"],
                                mode = "val",
                                transforms=eval_trans_seq)
    ...
    def _train(self, epoch_id):
        ...
        with tqdm(total=len(self.train_loader)) as pbar:
            for I_o, I_r, V_h, target in self.train_loader:
                I_o, I_r, V_h, target = I_o.to(self.device), I_r.to(self.device), V_h.to(self.device), target.to(self.device)
                self.optimizer.zero_grad()
                output = self.net(I_o, I_r, V_h)
    ...
    def _forward(self, data_loader):
        ...
        with torch.no_grad():
            with tqdm(total=num_steps) as pbar:
                for I_o, I_r, V_h, target in data_loader:
                    I_o, I_r, V_h, target = I_o.to(self.device), I_r.to(self.device), V_h.to(self.device), target.to(self.device)
                    output = F.softmax(net(I_o, I_r, V_h), dim=1)
                    loss = self.criterion(output, target)
